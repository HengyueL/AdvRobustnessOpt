...
def calc_min_dist_sample_fab(
    ...
    for key in keys:
        ...
        if check_feasibility:
            condition = distance_to_boundary > -1e-3
        else:
            condition = True
        if p_distance < best_radius and condition:
...
def main(cfg, default_dtype=torch.double):
    ...
    fab_opt_runner_full = FABAttackPTModified(
        classifier_model, n_restarts=1, n_iter=max_iter, targeted=False, eps=init_scale,
        seed=cfg["seed"], norm=attack_type, verbose=False, device=device
    )
    ...
    for batch_idx, data in enumerate(val_loader):
        if batch_idx < cfg["curr_sample"]:
            ...
            if pred_correct_before < 0.5:
                ...
                # OPT: Stage 1 --- Early Stopping
                fab_time_start = time.time()
                fab_adv_output_es, fab_best_iter_dict = fab_opt_runner_es.perturb(
                    inputs, labels
                )
                ...
                _, _, _, _, best_key = calc_min_dist_sample_fab(
                    fab_adv_output_es, inputs, attack_type, classifier_model, labels.item(), log_file, check_feasibility=False
                )
                es_time = fab_time_end - fab_time_start
                # === OPT: Stage 2 --- Continue with the best one
                x_init = fab_adv_output_es[best_key]
                iter_init = fab_best_iter_dict[best_key]
                t_start = time.time()
                fab_adv_output_full, fab_best_iter_dict_full = fab_opt_runner_full.perturb(
                    inputs, labels, x_init=x_init, iter_resume=iter_init
                )
                t_end = time.time()
                ...
                result_summary["true_label"].append(labels.item())
                result_summary["max_logit_before_opt"].append(pred_before.item())
                result_summary[attack_distance_key].append(distance_dict[final_key])
                result_summary["max_logit_after_opt"].append(attacked_label_dict[final_key])
                result_summary["distance_to_decision_boundary"].append(boundary_distance_dict[final_key])
                result_summary["box_constraint_violation"].append(box_violation_dict[final_key])
                result_summary["time"].append(t_end - t_start + es_time)
                result_summary["best_iter"].append(fab_best_iter_dict_full[final_key])
            ...
            if cfg["save_vis"]:
                ...
                adv_img_np = fab_adv_output_full[final_key].detach().cpu().numpy()[0, :, :, :]
