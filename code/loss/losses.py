...
class MarginLossOrig(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        if self.reduction == "none":
            loss = (max_incorrect_logits - correct_logits).clamp(max=self.clamp_value).squeeze(1)
        elif self.reduction == "sum":
            loss = (max_incorrect_logits - correct_logits).clamp(max=self.clamp_value).sum(dim=0)
        elif self.reduction == "mean":
            loss = (max_incorrect_logits - correct_logits).clamp(max=self.clamp_value).mean(dim=0)
        else:
...
class CELoss(nn.Module):
    def __init__(self, reduction="mean", use_clip_loss=False, clamp_value=10) -> None:
        ...
        if self.use_clip_loss:
            ...
            self.clamp_value = clamp_value = float("inf") 
    
    def forward(self, logits, labels):     
        if self.reduction == "none":
            loss = self.loss_func(logits, labels).clamp(max=self.clamp_value)
        elif self.reduction == "sum":
            loss = self.loss_func(logits, labels).clamp(max=self.clamp_value).sum(dim=0).unsqueeze(0)
        elif self.reduction == "mean":
            loss = self.loss_func(logits, labels).clamp(max=self.clamp_value).mean(dim=0).unsqueeze(0)
        else:
            raise RuntimeError("Unsupported reduction method.")
        return loss
...
class NegMarginLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        if self.reduction == "none":
            ...
            loss = (max_incorrect_logits - correct_logits).clamp(max=self.clamp_value).sum(axis=0)
        elif self.reduction == "mean":
            loss = (max_incorrect_logits - correct_logits).clamp(max=self.clamp_value).mean(axis=0)
        else:
            raise RuntimeError("Unsupported reduction method.")
        return -loss
